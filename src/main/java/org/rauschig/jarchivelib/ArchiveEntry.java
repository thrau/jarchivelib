 ...
 *
 * <p>The life of an {@link ArchiveEntry} is dependent on the status of the {@link ArchiveStream} it
 * came from. Once retrieved via {@link ArchiveStream#getNextEntry()}, the entry can be used as long
 * as the {@code ArchiveStream} remains on this entry, i.e. {@code getNextEntry()} was not called,
 * and the stream was not since closed.
 */
public interface ArchiveEntry {
   ...
   * @throws IllegalArgumentException if the destination is not a directory, or a directory can not
   *     be created at the given location
   */
  File extract(final File destination)
