...
/**
 * Factory for creating {@link Archiver} instances by a given archiver type name. Use the constants
 * in this class to pass to the factory method.
 */
public final class ArchiverFactory {
  ...
  /**
   * Probes the given {@link File} for its file type and creates an {@link Archiver} based on this
   * file type. If the File has a composite file extension such as ".tar.gz", the created {@link
   * Archiver} will also handle ".gz" compression.
   *
   ...
   */
  public static Archiver createArchiver(final File archive) throws IllegalArgumentException {
    final FileType fileType = FileType.get(archive);
  ...
  /**
   * Creates an Archiver that handles the given {@link FileType}. The Archiver may handle
   * compression inherently, if the {@link FileType} uses a compression type, such as ".tgz" might.
   *
   * @param fileType the file type
   * @return a new Archiver instance (that may also handle compression)
   */
  public static Archiver createArchiver(final FileType fileType) {
    if (fileType == FileType.UNKNOWN) {
   ...
   */
  public static Archiver createArchiver(final String archiveFormat, final String compression)
   ...
   * @return a new Archiver instance that also handles compression
   */
  public static Archiver createArchiver(final ArchiveFormat archiveFormat, final CompressionType compression) {
    final CommonsArchiver archiver = new CommonsArchiver(archiveFormat);
    final CommonsCompressor compressor = new CommonsCompressor(compression);
   ...
   */
  public static Archiver createArchiver(final String archiveFormat) throws IllegalArgumentException {
    if (!ArchiveFormat.isValidArchiveFormat(archiveFormat)) {
      throw new IllegalArgumentException("Unknown archive format " + archiveFormat);
    }
   ...
   * @return a new Archiver instance
   */
  public static Archiver createArchiver(final ArchiveFormat archiveFormat) {
